 ...
 */
function wph_jo_theme_health_checker() {
	...
	// compare php version
	$error_message = wp_kses_post( __( 'You need to upgrade your PHP version. Required <b>at least %s</b>, your current: <b>%s</b>', 'jo-theme' ) );
	if ( version_compare( phpversion(), $min_php_version, '<' ) ) {
		$errors[] = sprintf( $error_message, $min_php_version, phpversion() );
	}

	// check folder rights
	$error_message = wp_kses_post( __( 'Set 0777 permissions to <b>%s</b> folder.', 'jo-theme' ) );
	...
	// check php settings
	$error_message = wp_kses_post( __( 'PHP setting <b>%s</b> must have at least <b>%s</b> value. Your current: <b>%s</b>.', 'jo-theme' ) );
	foreach ( $min_php_settings as $setting => $min_value ) {
		$current_value   = wph_jo_return_bytes( ini_get( $setting ) );
		$min_value_bytes = wph_jo_return_bytes( $min_value );
	...
	wph_jo_display_theme_health_notification( $errors );
}
 ...
 * @param $errors
 */
function wph_jo_display_theme_health_notification( $errors ) {
	...
	$markup .= esc_attr__( 'WPHunters Health Checker have detected next problems:', 'jo-theme' );
	...
	$markup .= '</ul><p>';
	$markup .= wp_kses_post( __( 'To use theme please fix these issues. <i>This message visible only for site administrators.</i>', 'jo-theme' ) );
	$markup .= '</p>';
	$markup .= '<p>' . sprintf( __( '<a href="http://docs.wphunters.com/article/282">Read more about this message and how to fix mentioned errors.</a>', 'jo-theme' ) ) . '</p>';
	$markup .= '</div>';
 ...
 * @return int|string
 */
function wph_jo_return_bytes( $val ) {
