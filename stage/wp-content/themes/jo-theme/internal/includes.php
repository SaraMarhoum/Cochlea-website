...
class WPH_Theme_Includes {
	...
	public static function init() {
		 ...
		 */
		{
			...
			self::include_child_first( '/vc_templates.php' );
			self::include_child_first( '/envira.php' );

			self::include_all_child_first( '/includes' );
			self::include_all_child_first( '/vc_widgets' );
		 ...
		 */
		if ( ! is_admin() ) {
			// Include later to be able to make wp_dequeue_style|script()
	...
	private static function get_rel_path( $append = '' ) {
		if ( self::$rel_path === null ) {
			self::$rel_path = '/' . basename( WPH_THEMEDIR . '/internal' );
		}
	 ...
	 * Child theme listed first.
	 *
	 * @param $dir_rel_path
	 */
	public static function include_all_child_first( $dir_rel_path ) {
		$paths = array();
	...
	/**
	 * Get path relative to parent theme
	 *
	 * @param $rel_path
	 *
	 * @return string
	...
	/**
	 * Get path relative to child theme
	 *
	 * @param $rel_path
	 *
	 * @return null|string
	...
	/**
	 * Process both file versions - from child and from parent
	 *
	 * @param $rel_path
	 */
	...
	public static function _action_widgets_init() {
		...
		if ( is_child_theme() ) {
			$paths[] = self::get_child_path( '/wp_widgets' );
		}

		$paths[] = self::get_parent_path( '/wp_widgets' );
		...
		foreach ( $paths as $path ) {
			...
			foreach ( $dirs as $dir ) {
				...
				$widget_class = 'WPH_JO_Widget_' . self::dirname_to_classname( $dirname );
