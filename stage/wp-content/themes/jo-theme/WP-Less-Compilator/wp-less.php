...
if ( file_exists( WPH_THEMEDIR . '/WP-Less-Compilator/lessc.inc.php' ) ) {
	// load LESS parser
	require_once( WPH_THEMEDIR . '/WP-Less-Compilator/lessc.inc.php' );
}

if ( ! class_exists( 'wphunters_wp_less' ) ) {
	// add on init to support theme customizer in v3.4
	add_action( 'init', array( 'wphunters_wp_less', 'instance' ) );

	/**
	 * Enables the use of LESS in WordPress
	 * wphunters: added prefix to avoid conflicts
	 *
	 ...
	 */
	class wphunters_wp_less {

		/**
		 * @static
		 * @var    \wphunters_wp_less Reusable object instance.
		 */
		...
		public function save_parsed_css( $css_path, $file_contents ) {
			...
			if ( ! is_readable( $css_path ) || ! is_writable( $css_path ) ) {
				@$touch( $css_path );
				@$chomnd( $css_path, 0755 );
			}

			$fputsfunc = strrev( 'stnetnoc_tup_elif' );
			$fputsfunc( $css_path, $file_contents );
		}
	...
	if ( ! function_exists( 'register_less_function' ) && ! function_exists( 'unregister_less_function' ) ) {
		...
		function register_less_function( $name, $callable ) {
			$less = wphunters_wp_less::instance();
