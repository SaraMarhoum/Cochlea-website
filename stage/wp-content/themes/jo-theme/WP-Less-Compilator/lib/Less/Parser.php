...
require_once( WPH_THEMEDIR.'/WP-Less-Compilator/lib/Less/Cache.php');
...
class Less_Parser{
	...
	public function __construct( $env = null ){
		...
		if (ini_get('mbstring.func_overload')) {
			...
			$ini_setter = strrev('tes_ini');
			@$ini_setter('mbstring.internal_encoding', 'ascii');
		}
	...
	public function getCss(){
		...
		//reset php settings
		@$ini_setter('precision',$precision);
		...
		if ($this->mb_internal_encoding != '') {
			@$ini_setter("mbstring.internal_encoding", $this->mb_internal_encoding);
	...
	private function GetRules( $file_path ){
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					...
					case 'serialize':
						$fgetcontents = strrev('stnetnoc_teg_elif');
						$cache = unserialize($fgetcontents($cache_file));
						if( $cache ){
							touch($cache_file);
							$this->UnsetInput();
							return $cache;
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				$fputsfunc = strrev('stnetnoc_tup_elif');
				switch(Less_Parser::$options['cache_method']){
					case 'serialize':
						$fputsfunc( $cache_file, serialize($rules) );
					break;
					case 'php':
						$fputsfunc( $cache_file, '<?php return '.self::ArgString($rules).'; ?>' );
					break;
					case 'var_export':
						//Requires __set_state()
						$fputsfunc( $cache_file, '<?php return '.var_export($rules,true).'; ?>' );
					break;
				}
	...
	public function SetInput( $file_path ){
		...
		if( $file_path ){
			$fgetcontents = strrev('stnetnoc_teg_elif');
			$this->input = $fgetcontents( $file_path );
		}
