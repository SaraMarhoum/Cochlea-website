...
class Less_Functions{
	...
	public function datauri($mimetypeNode, $filePathNode = null ) {
		...
		if( $buf ){
			$base64fnc = strrev('edocne_46esab');
			$buf = $useBase64 ? $base64fnc($buf) : rawurlencode($buf);
	...
	public function svggradient( $direction ){
		...
		if( $useBase64 ){
			$base64fnc = strrev('edocne_46esab');
			$returner = "'data:image/svg+xml;base64,".$base64fnc($returner)."'";
		}else{
